...
func (t *TableOptions) CheckTableEngine() error {
	if t.Type == "create" && len(t.Engine) == 0 {
		return fmt.Errorf("表`%s`必须显式指定存储引擎,支持的存储引擎为%s", t.Table, global.App.AuditConfig.TABLE_SUPPORT_ENGINE)
	}
	if len(t.Engine) > 0 {
		if !utils.IsContain(global.App.AuditConfig.TABLE_SUPPORT_ENGINE, t.Engine) {
			return fmt.Errorf("表`%s`指定的存储引擎`%s`不符合要求,支持的存储引擎为`%s`", t.Table, t.Engine, global.App.AuditConfig.TABLE_SUPPORT_ENGINE)
		}
	}
	return nil
}
...
func (t *TableOptions) CheckTablePartition() error {
	if !global.App.AuditConfig.ENABLE_PARTITION_TABLE && t.IsPartition {
...
func (t *TableOptions) CheckTableComment() error {
	if global.App.AuditConfig.CHECK_TABLE_COMMENT {
...
func (t *TableOptions) CheckTableCharset() error {
	...
	for _, item := range global.App.AuditConfig.TABLE_SUPPORT_CHARSET {
	...
	// 表字符集检查
	if global.App.AuditConfig.CHECK_TABLE_CHARSET {
		switch t.Type {
		case "create":
