...
func LogicCreateTableIsExist(v *traverses.TraverseCreateTableIsExist, r *hint.RuleHint) {
...
// LogicCreateTableAs
func LogicCreateTableAs(v *traverses.TraverseCreateTableAs, r *hint.RuleHint) {
	if v.IsCreateAs {
		// 不深入检查AS后面的语法
...
func LogicCreateTableLike(v *traverses.TraverseCreateTableLike, r *hint.RuleHint) {
	if v.IsCreateLike {
...
func LogicCreateTableOptions(v *traverses.TraverseCreateTableOptions, r *hint.RuleHint) {
	v.Type = "create"
...
func LogicCreateTablePrimaryKey(v *traverses.TraverseCreateTablePrimaryKey, r *hint.RuleHint) {
	// 必须定义主键
...
func LogicCreateTableConstraint(v *traverses.TraverseCreateTableConstraint, r *hint.RuleHint) {
...
func LogicCreateTableAuditCols(v *traverses.TraverseCreateTableAuditCols, r *hint.RuleHint) {
...
func LogicCreateTableColsOptions(v *traverses.TraverseCreateTableColsOptions, r *hint.RuleHint) {
...
func LogicCreateTableColsRepeatDefine(v *traverses.TraverseCreateTableColsRepeatDefine, r *hint.RuleHint) {
	// 查找重复的列名
...
func LogicCreateTableColsCharset(v *traverses.TraverseCreateTableColsCharset, r *hint.RuleHint) {
	// 列字符集检查
...
func LogicCreateTableIndexesPrefix(v *traverses.TraverseCreateTableIndexesPrefix, r *hint.RuleHint) {
...
func LogicCreateTableIndexesCount(v *traverses.TraverseCreateTableIndexesCount, r *hint.RuleHint) {
	// 检查二级索引数量
...
func LogicCreateTableIndexesRepeatDefine(v *traverses.TraverseCreateTableIndexesRepeatDefine, r *hint.RuleHint) {
	// 查找重复的索引
...
func LogicCreateTableRedundantIndexes(v *traverses.TraverseCreateTableRedundantIndexes, r *hint.RuleHint) {
...
func LogicCreateTableDisabledIndexes(v *traverses.TraverseCreateTableDisabledIndexes, r *hint.RuleHint) {
	// BLOB/TEXT类型不能设置为索引
...
func LogicCreateTableInnodbLargePrefix(v *traverses.TraverseCreateTableInnodbLargePrefix, r *hint.RuleHint) {
...
func LogicCreateTableInnoDBRowSize(v *traverses.TraverseCreateTableInnoDBRowSize, r *hint.RuleHint) {
...
func LogicCreateTableInnoDBRowFormat(v *traverses.TraverseCreateTableOptions, r *hint.RuleHint) {
	// 判断行格式
	var rowFormat string = v.RowFormat
	if v.RowFormat == "DEFAULT" {
		innodbDefaultRowFormat, ok := r.KV.Get("innodbDefaultRowFormat")
		if !ok {
			r.Summary = append(r.Summary, "无法获取innodb_default_row_format参数")
			return
		}
		rowFormat = innodbDefaultRowFormat
	}
	if !utils.IsContain(r.AuditConfig.INNODB_ROW_FORMAT, rowFormat) {
		r.Summary = append(r.Summary, fmt.Sprintf("表`%s`允许设置的行格式为%s，当前ROW_FORMAT=%s", v.Table, strings.Join(r.AuditConfig.INNODB_ROW_FORMAT, ","), rowFormat))
	}
}
