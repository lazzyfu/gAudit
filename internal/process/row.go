...
func (rs *InnoDBRowSize) Check(kv *kv.KVCache[string]) error {
	if rs.Engine != "InnoDB" {
		return nil
	}
	...
	innodbDefaultRowFormat, ok := kv.Get("innodbDefaultRowFormat")
	if !ok {
		return fmt.Errorf("无法获取innodb_default_row_format参数")
	}
	if rowFormat == "DEFAULT" {
		rowFormat = strings.ToUpper(innodbDefaultRowFormat)
	}
