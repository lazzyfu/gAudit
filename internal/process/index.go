...
func (l *LargePrefix) Check(kv *kv.KVCache[string]) error {
	...
	versionIns := DbVersion{Version: dbVersion}

	indexMaxLength := defaultLargePrefixLimit
	largePrefix, ok := kv.Get("largePrefix")
	if !ok {
		return fmt.Errorf("无法获取innodb_large_prefix参数")
	}

	if versionIns.IsTiDB() || (versionIns.Int() > 80000) || (versionIns.Int() > 50700 && largePrefix == "ON") {
