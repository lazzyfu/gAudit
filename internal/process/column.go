...
func (c *ColOptions) CheckColumnMaxVarcharLength() error {
	if c.AuditConfig.MAX_VARCHAR_LENGTH < c.Flen && c.Tp == mysql.TypeVarchar {
		return fmt.Errorf("列`%s`长度太大（max = %d），当前varchar长度为%d，请使用BLOB或TEXT类型替代[表`%s`]", c.Column, c.AuditConfig.MAX_VARCHAR_LENGTH, c.Flen, c.Table)
	}
	return nil
}
...
func CheckColsTypeChanged(col ColOptions, vCol ColOptions, auditConfig *config.AuditConfiguration, kv *kv.KVCache[string], tp string, table string) error {
	// tp = modify or change
	...
	if !ok {
		return fmt.Errorf("无法获取数据库版本信息")
	}
	dbVersionIns := DbVersion{Version: dbVersion}
